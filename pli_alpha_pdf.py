...
# ========== CONFIGURATION ==========
# Set the folder where your PLI Alpha result files are located
BASE_PATH = r"C:\Users\mohimaCHAKRABORTY\Taskupdate\machine_learning\results\PLI\alpha"

# File names (they should be present in BASE_PATH)
...
OUTPUT_PDF = os.path.join(BASE_PATH, "PLI_alpha_classification_report.pdf")
...
def dataframe_to_table(df, title, ax, fontsize=9):
    ...
    if isinstance(df, pd.Series):
        df = df.to_frame()
    # Convert to string with 4 decimals for numeric columns
    df_display = df.copy()
...
# ========== MAIN ==========
print("Generating PDF report for PLI Alpha band...")
print(f"Looking for files in: {BASE_PATH}")

# Change working directory to BASE_PATH (or join paths)
...
with PdfPages(OUTPUT_PDF) as pdf:
    ...
    if all(c in model_comp.columns for c in display_cols):
        summary_df = model_comp[display_cols].copy()
    else:
        summary_df = model_comp
    ...
    for clf in classifier_reports.keys():
        df_report = classifier_reports[clf]
