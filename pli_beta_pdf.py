...
# ========== CONFIGURATION ==========
# Set the folder where your PLI Beta result files are located
BASE_PATH = r"C:\Users\mohimaCHAKRABORTY\Taskupdate\machine_learning\results\PLI\beta"

# File names (they must exist in BASE_PATH)
...
OUTPUT_PDF = os.path.join(BASE_PATH, "PLI_beta_classification_report.pdf")
...
def dataframe_to_table(df, title, ax, fontsize=9):
    """Render a DataFrame as a table on a given matplotlib axis."""
    ax.axis('tight')
    ax.axis('off')
    if isinstance(df, pd.Series):
        df = df.to_frame()
    # Format numeric values to 4 decimal places (if numeric)
    df_display = df.copy()
...
# ========== MAIN ==========
print("Generating PDF report for PLI Beta band...")
...
with PdfPages(OUTPUT_PDF) as pdf:
    # Page 1: Model Comparison (mean ± std)
    ...
    # Pages for each classifier (only those with report)
    for clf, df_report in classifier_reports.items():
        ...
        # Select rows of interest (0,1, macro avg, weighted avg)
